...
#def load_bytes(file_path: str) -> str:
#    with open(file_path, 'r') as file:
#        return file.read()

def load_data(table: str, tick: str) -> pd.DataFrame:
    path = f'{DATA_FOLDER}/{table}/{tick}.ftr'
    return pd.read_feather(path)

#def load_json(table_name: str, tick: str) -> dict:
#    return json.loads(load_bytes(f'./data/alpha_vantage/{table_name}/{tick}.json'))
...
def load_fundamentals(table_name: str, tick: str, use_reported_date: bool=False) -> pd.DataFrame:
    ...
    data=load_data(table_name, tick)
...
def load_price_data(tick_list: list[str]) -> pd.DataFrame:
    price_data = []
    for tick in tick_list:
        tick_data = load_data('price_data', tick)
